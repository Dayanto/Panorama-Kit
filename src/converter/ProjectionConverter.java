...
/**
 * This converter loads a panorama made with one projection and converts it to another projection. The 
 * position of a pixel is traced to a location in the original image, but with all the warping involved, 
 * most of the time, pixels in one of the projections will not line up perfectly with the pixels in the 
 * other. Instead, the pixels in the output image are being mapped to a floating point position perfectly 
 * matching where a pixel should have been. The color of this virtual pixel is determined by looking at 
 * the surrounding pixels and sampling their colors. How close the surrounding pixels are to the virtual
	...
	public void convert() throws IOException, Exception
	{
		...
		for(int xOutput = 0; xOutput < outputImage.getWidth(); xOutput++)
		{
			...
			{	
				...
				PixelCoordinate[][] pixelData = positionMapper.getPixelCoordinates(position, inputImage.getWidth(), inputImage.getHeight(), interpolator.sampleSize);
