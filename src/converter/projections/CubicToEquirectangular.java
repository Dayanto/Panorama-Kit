...
import converter.samplers.CubicSampler;
import converter.PositionMapper;
import converter.data.Position;

/**
 * This projection takes any point in an equirectangular projection and returns the position that 
 * point would match in the cubic projection. The cubic projection uses two different positioning 
 * systems. The first, what I call the "equator" positioning uses mostly straight lines where moving 
 * along the x or y axis in the equrectangular projection mostly follows the same axis in the cubic. 
 * In the "polar" positioning system instead, the x and y positions are determined by the distance 
 * from a center point in one of two specific regions of the image, as well as the rotation around 
 * that point. 
 * 
...
public class CubicToEquirectangular extends PositionMapper
{
	...
	{	
		...
		// circumferance in pixels and then converting to radians. The additional
		// half pixel comes from the fact that pixel positions (indices) are numbered
		// by their top left corner while the trigonometric functions need to work
		// with their center points. This half pixel is later returned.
		double horizontalAngle = (xCenteredLocal + 0.5) * ((2 * Math.PI) / inputWidth);
		double verticalAngle = (yCentered + 0.5) * ((2 * Math.PI) / inputWidth);
		...
		// The calculations are  based on the idea of a sphere inside of a 
		// cube and following the straight lines from the center through the 
		// sphere and to the matching point on the outside cube.
		...
		{
			// Which pole we're at currently. Represents a value of 1 or -1
			// where -1 is the pole on top and 1 is the one on the bottom. Since 
			// 1 doesn't change anything when multiplied, all calculations will 
			// be based off of the bottom pole and flipped for the top.
			int hemisphere = (int) (verticalAngle / Math.abs(verticalAngle)); 
			
			// Calculate the relative position from the pole center. The rotation
			// is counter clockwise and starts pointing straight left.
