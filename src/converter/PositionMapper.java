...
/**
 * A position mapper is used to track the position from one projection to another in the form of a floating point
 * position. The tracking occur backwards; it starts at the final position in the output image and tracks the position
 * to sample from in the original image. It is possible to stack multiple projections sequentially. Doing this will
 * simply carry the floating point position through another projection before any pixels are sampled, so that no image 
 * quality is lost.
 *
 * @author dayanto
